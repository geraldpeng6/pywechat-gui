...
from .models import ResourceExportKind, ResourceExportRequest, ResourceExportResult, RuntimeOptions, coerce_resource_export_kind
...
def export_kind_label(kind: ResourceExportKind | str) -> str:
    normalized = coerce_resource_export_kind(kind)
    mapping = {
        ResourceExportKind.RECENT_FILES: "最近聊天文件",
        ResourceExportKind.WXFILES: "微信聊天文件",
        ResourceExportKind.VIDEOS: "微信聊天视频",
    }
    if not isinstance(normalized, ResourceExportKind):
...
class ResourceExportService:
    ...
    def run_export(
        ...
        export_kind = coerce_resource_export_kind(request.export_kind)
        if not isinstance(export_kind, ResourceExportKind):
            raise ValueError("请选择有效的导出类型")

        if export_kind is ResourceExportKind.RECENT_FILES:
            if on_progress:
                on_progress("正在导出最近聊天文件...")
            exported = self.adapter.export_recent_files(str(target_folder), runtime_options)
        elif export_kind is ResourceExportKind.WXFILES:
            if on_progress:
        ...
        summary_path = target_folder / f"{export_kind.value}-summary-{datetime.now().strftime('%Y%m%d-%H%M%S')}.txt"
